...
parser.add_argument('--save-path', default='./logs/RGB_Depth/Train/', type=str, help='logs save path')
...
def train(epochs):
    out_dir = './models/output/RGB_Depth'
    ...
    logger.Print(f"|---AUC history---|")
    for i in range(len(auc_history)):
        logger.Print(auc_history[i])
