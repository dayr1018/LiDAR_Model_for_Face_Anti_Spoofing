...
def test(args, dataloader):
    ...
    # # 1
    # fileset = {}    
    # fileset["300"] = "epoch_300_model.pth"
    ...
    for epoch, filename in fileset.items():
        ...
        # (rgb, rgbdp_v3, rgbdp_v2, rgbd_v1, rgbd_v2) uses this. 
...
if __name__ == "__main__":
    ...
    #다섯 가지 경우의 Acc, F1 정리 
    loader_types =  ["Indoor", "Outdoor", "Indoor(dark)"]
    test_performs = [[] for i in range(len(loader_types))]
      
    dataloaders = [type1_loader, type2_loader, type3_loader]
    for i, loader in enumerate(dataloaders):
        writer = SummaryWriter(f"runs/ieee_access_final_1103/{args.message}_{args.attacktype}_{loader_types[i]}")
        # writer = SummaryWriter(f"runs/test/{args.message}_{args.attacktype}_{loader_types[i]}")
